# ------------------------------------------------------------------------------
#  Read in Germany's expenditure components of GDP and plot shares
...
# get data (kludgy) 
data <- as.data.frame(t(input[2:8,4:37]))
colnames(data) <- c("Y", "C", "G", "I", "S", "X", "M")
data$date <- c(1980:2013)
...
# check GDP re unification
plot(data$date, data$Y, type="l", cex=1.5, lwd=2, col="blue")
dev.print(device=pdf, file="Germany_GDP.pdf",width=8,height=6)
...
abline(0,0)
mtext("Germany", side=3, adj=0, line=1.0, cex=1.25)
text(1985, 0.30, "Saving", cex=1, adj=0)
text(1985, 0.18, "Investment", cex=1, adj=0)
text(1985, 0.08, "Net Exports", cex=1, adj=0)
...
dev.print(device=pdf, file="Germany_shares.pdf",width=8,height=6)
