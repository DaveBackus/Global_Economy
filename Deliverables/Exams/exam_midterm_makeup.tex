...
\newcommand{\hsp}{\phantom{--}}

\def\ClassName{The Global Economy}
\def\Category{Professor David Backus}
\def\HeadName{Makeup Midterm Examination}
...
\begin{enumerate}
% ======================================================================
\item {\it China and India.\/}  China and India have been among
the fastest growing countries in the world over the last two
decades.  Your task is to compare their experiences.
%
\begin{center}
\tabcolsep = 0.05in
...
\hline\hline
China   &  1985   & 2,530  & 3,011  &  4.9        \\
China   &  2000   & 6,175  & 9,190  &  6.4        \\
India   &  1985   & 3,546  & 3,833  &  3.6        \\
India   &  2000   & 6,216  & 6,414  &  5.1   \\
\hline\hline
\end{tabular}
\end{center}
...
\item Which country had higher total factor productivity in 2000?
(10~points)

\item Which country's output per worker grew more rapidly between
1985 and 2000?  (10~points)

\item Which factors are mostly responsible for the difference in
...
Answer.
\begin{enumerate}
\item We compute TFP from $ A = (Y/L)/[(K/L)^\alpha H^{1-\alpha} ] $. Using $\alpha = 1/3$, the
answers are 85.5 (China) and 112.9 (India).

\item Continuously-compounded growth rates of output per worker were 5.95\% (China) and 3.74\%
(India).  As an example, the first is computed from
\[
    \gamma_{Y/L} \;=\; \frac{\log(6175/2530)}{2000-1985} 
                \;=\; 0.0595 .
\]
...
\begin{eqnarray*}
    \mbox{China:} &&  5.95 \;=\; 2.28 \mbox{ (A)} +  2.48 \mbox{ (K/L)} + 1.19 \mbox{ (H)}  \\
    \mbox{India:} &&  3.74 \;=\; 1.05 \mbox{ (A)} +  1.14 \mbox{ (K/L)} + 1.55 \mbox{ (H)} .
\end{eqnarray*}

In words:  in India educational attainment ($H$) has been growing
at a higher rate, but China scored better in terms of growth of
...
high numbers indicate more corruption.    
%
\begin{enumerate}
...
An analyst at the OECD commented that the difficulty Italy has had with increasing international trade was a reflection of its rigid labor markets, 
which discourage Italian businesses from the restructuring needed to 
compete in global markets.
One byproduct, she suggested, was a decline in TFP. 
Do you find this argument persuasive?   Why or why not?  (10~points)

\item {\it PPP-adjusted data.\/}
In China, investment is 40\% of GDP when computed with 
domestic prices, 
but only 20\% of GDP when we use international prices 
(so-called PPP-adjusted data).  
Why are the two so different?  
Is the difference a source of concern for China?   
(10~points)
