...
Some of these are more complicated than I'd like.
For example, the capital-labor ratio is computed from 
\begin{eqnarray*}
    K/L &=& \frac {K/Y} {Y/\POP} \ {(L/\POP)} .
\end{eqnarray*}

TFP is computed from the production function
\begin{eqnarray*}
    Y/L  &=& A (K/L)^{1/3} ,
\end{eqnarray*}
so that $ A = Y/(K/L)^{1/3} $.
...
That reflects, in part, a large disparity in
...
\item Both countries have some labor market issues.
...
%**********************************************************************
\newpage
\def\HeadName{Practice Midterm Examination \#4}
\parindent = 0.0in
\parskip = \bigskipamount
%\setcounter{page}{1}
\thispagestyle{empty}
...
\item What is the average annual growth rate of GDP per capita between
2000 and 2011?
GDP per worker?
...
\item What was total factor productivity in 2000 and 2011?
Its average annual growth rate?
(10~points)

\item What are the other sources of growth?
...
\item Productivity we find indirectly from
$ A = (Y/L)/(K/L)^\alpha$.
In 2000, we find
\begin{eqnarray*}
...
\item Growth in GDP per worker has these components:
\begin{eqnarray*}
...
\begin{solution}
\begin{parts}
\item All of these matter somewhat.  I'd say
...
\item One thing you might do is rate the two countries along all
of these dimensions,
...
%
\begin{parts}
\item If a product is made in the Mexico but sold to consumers in the US,
it is not included in Mexican GDP.
(10~points)

\item If the unemployment rate falls, employment has risen.
(10~points)

\item Firms find it costly to search for workers with the right skills.
For that reason, regulations that discourage labor turnover are good for the economy.
(10~points)

\item A tax on labor tends to reduce employment.
(10~points)

\item In Ricardo's model, free trade is good for consumers but bad for workers.
(10~points)
\end{parts}

\begin{solution}
\begin{parts}
\item False.  GDP measures production in a country.  It doesn't matter who buys it.
\item Uncertain.  The issue here is that there are three categories:
...
\item True.  Think of a supply/demand setup.
